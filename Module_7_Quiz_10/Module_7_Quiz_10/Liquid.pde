public class Liquid
{
  ...
  public boolean isCollidingWith(Walker walkers)
  {
    PVector pos = walkers.position;
    return pos.x > this.x - this.horizon &&
           pos.x < this.x + this.horizon &&
           pos.y < this.y;
  }
  
  public PVector calculateDragForce(Walker walkers)
  {
    //drag magnitude is coefficient of drag * speed squared
    float speed = walkers.velocity.mag();
    float dragMagnitude = this.cd * speed * speed;
    
    //direction is inverse of velocity
    PVector dragForce = walkers.velocity.copy().mult(-1);
